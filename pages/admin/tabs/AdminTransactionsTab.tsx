...
export const AdminTransactionsTab: React.FC = () => {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-3xl font-bold text-white">Global Transaction History</h1>
                                    ...
                                    <div key={tx.id} className="bg-brand-dark/30 p-4 rounded-lg border border-brand-ui-element/20">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <p className="text-xs text-brand-ui-element/80 mt-1">{tx.description}</p>
                                            </div>
                                            <p className={`font-mono text-lg shrink-0 ml-4 ${tx.amount > 0 ? 'text-success' : tx.amount < 0 ? 'text-error' : ''}`}>
                                ...
                                <tbody>
                                    {transactions.map(tx => (
                                        <tr key={tx.id} className="border-b border-brand-ui-element/20 last:border-0 hover:bg-brand-ui-element/10">
                                            ...
                                            <td className={`p-4 font-mono text-right ${tx.amount > 0 ? 'text-success' : tx.amount < 0 ? 'text-error' : ''}`}>
                                                {tx.amount > 0 ? '+' : ''}£{Math.abs(tx.amount).toFixed(2)}
                                            </td>
